import { tilesEqual, arrange, openingValue, findLayablePairs, orderMeldForDisplay, meldRepresentedValues, meldRepresentedColors } from '@cs-okey/engine'
import type { Tile, TileColor } from '@cs-okey/engine'
import type { VariantConfig } from '@cs-okey/engine'

// The okey "oracle" + display ordering now live in the engine (single source of
// truth shared with the lay-off rule). Re-export so existing app imports
// (CenterMelds, TableMelds, tests) from '../rack/slots' keep working.
export { orderMeldForDisplay, meldRepresentedValues, meldRepresentedColors }
